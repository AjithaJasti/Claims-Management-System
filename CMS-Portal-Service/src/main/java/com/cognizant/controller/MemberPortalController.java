...
public class MemberPortalController {
	...
	public String MemberPortal(@RequestParam("username") String username, @RequestParam("password") String password,Model model,HttpSession session) {
		...
		try {
			response = restTemp.postForEntity( "http://cms-authorization-service/api/auth/login", creds , UserToken.class );
		} catch (Exception e) {
	...
	public String viewBills(@RequestParam("memberId") String memberId, @RequestParam("policyId") String policyId,
		...
		if(valid=="True") {
			...
			ResponseEntity<ViewBillResponse[]> response  = restTemp.exchange("http://cms-member-service/viewbill/" + memberId + "/" + policyId, HttpMethod.GET, requestEntity, ViewBillResponse[].class); 
		...
		if(valid=="True") {
			try {
				ResponseEntity<ProviderResult> response = restTemp
						.getForEntity("http://cms-policy-service/getChainOfProviders/" + policyId, ProviderResult.class);
